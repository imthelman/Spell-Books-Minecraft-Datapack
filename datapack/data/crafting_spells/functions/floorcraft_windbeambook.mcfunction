...
execute as @e[type=item,nbt={Item:{id:"minecraft:enchanted_book",tag:{wind_magic_shard:1b}}}] at @s run execute as @e[type=item,nbt={Item:{id:"minecraft:book",Count:10b}},distance=0..1,limit=1,sort=nearest] at @s run summon item ~ ~ ~ {Tags:["fresh_craft"],Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Windbeam Spellbook","color":"red","bold":true,"italic":false}',Lore:['{"text":"Spellbook that shoots wind gusts that luanchs enemys"}']},HideFlags:7,Unbreakable:1b,CustomModelData:10002,element:4,spell_id:1,Enchantments:[{id:"minecraft:vanishing_curse",lvl:1s}]}}}

#Kill the crafted_items
execute as @e[tag=fresh_craft] at @s run kill @e[type=item,nbt={Item:{id:"minecraft:enchanted_book",tag:{wind_magic_shard:1b}}},sort=nearest,limit=1]
