...
execute as @e[type=item,nbt={Item:{id:"minecraft:enchanted_book",tag:{lightning_magic_shard:1b}}}] at @s run execute as @e[type=item,nbt={Item:{id:"minecraft:book",Count:15b}},distance=0..1,limit=1,sort=nearest] at @s run execute as @e[type=item,nbt={Item:{id:"minecraft:lightning_rod",Count:1b}},distance=0..1,limit=1,sort=nearest] at @s run summon item ~ ~ ~ {Tags:["fresh_craft"],Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Lightningbeam Spellbook","color":"red","bold":true,"italic":false}',Lore:['{"text":"Spellbook that summons lightning beams"}']},HideFlags:7,Unbreakable:1b,CustomModelData:10002,element:2,spell_id:2,Enchantments:[{id:"minecraft:vanishing_curse",lvl:1s}]}}}
...
execute as @e[tag=fresh_craft] at @s run kill @e[type=item,nbt={Item:{id:"minecraft:lightning_rod",Count:1b}},limit=1,sort=nearest]
